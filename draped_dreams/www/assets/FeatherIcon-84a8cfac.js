	...
	x = {
		async register(e) {
			...
			try {
				...
				const s = await fetch(`${l}.register_user`, {
					method: "POST",
				...
				if (
					...
					console.log("Response headers:", s.headers),
					!s.ok)
				)
					return (
						console.error("HTTP error:", s.status, s.statusText),
						{ success: !1, message: `HTTP ${s.status}: ${s.statusText}` }
					);
				let a;
				try {
					(a = await s.json()), console.log("Response data:", a);
				} catch (t) {
