...
import { a as D } from "./api-b634d5e6.js";
const q = { class: "min-h-screen bg-gray-50" },
	...
	xe = { class: "flex items-center justify-between mb-3" },
	...
	Pe = { key: 3, class: "text-center py-12" },
	Ve = {
		__name: "Products",
		setup(Se) {
			const h = i([]),
				...
				C = i(!1),
				x = i(""),
				P = i(["Silk", "Cotton", "Designer", "Wedding", "Casual", "Party"]),
				b = i([]),
				B = W(() => {
					let t = [...b.value];
					switch (
						(d.value &&
							(t = t.filter(
								(e) =>
									e.product_name.toLowerCase().includes(d.value.toLowerCase()) ||
									e.description.toLowerCase().includes(d.value.toLowerCase())
							)),
						g.value && (t = t.filter((e) => e.category === g.value)),
						p.value &&
							(t = t.filter((e) => {
								const l = e.price;
								switch (p.value) {
						...
						case "price-low":
							t.sort((e, l) => e.price - l.price);
							break;
						case "price-high":
							t.sort((e, l) => l.price - e.price);
							break;
						case "newest":
							t.sort((e, l) => new Date(l.created_date) - new Date(e.created_date));
							break;
						case "popular":
							t.sort((e, l) => (l.rating || 0) - (e.rating || 0));
							break;
						case "featured":
							t.sort((e, l) =>
				...
				J = (t) => {
					const e = y.value.findIndex((l) => l.name === t.name);
					e > -1 ? y.value.splice(e, 1) : y.value.push(t),
						localStorage.setItem("wishlist", JSON.stringify(y.value));
				},
				N = (t) => y.value.some((e) => e.name === t),
				M = () => {
					(d.value = ""), (g.value = ""), (p.value = ""), (v.value = "featured");
				},
				w = () => {},
				$ = async () => {
					(C.value = !0), (x.value = "");
						...
						console.log("API result:", t),
							t.success
								? (console.log("Raw products data:", t.data),
								  ...
								  console.log("Processed products:", b.value))
								: ((x.value = t.message), console.error("API error:", t.message));
					} catch (t) {
						(x.value = "Failed to load products"),
				...
				O = async () => {
					try {
						...
						const t = await D.getCategories();
						console.log("Categories API result:", t),
							t.success &&
								((P.value = t.data), console.log("Loaded categories:", P.value));
					} catch (t) {
			...
			return (
				H(async () => {
					...
					e && (y.value = JSON.parse(e)),
						...
						await Promise.all([$(), O()]),
						console.log("Initialization complete. Products loaded:", b.value.length);
				}),
				(t, e) => {
					const l = k("FeatherIcon"),
						f = k("Button"),
						R = k("TextInput"),
						A = k("Badge");
					return (
							...
							s("main", te, [
								s("nav", se, [
									a(
										...
										{
											default: c(() => [
												a(l, { name: "home", class: "w-4 h-4 mr-2" }),
												e[9] || (e[9] = m(" Home ", -1)),
											]),
											_: 1,
										}
