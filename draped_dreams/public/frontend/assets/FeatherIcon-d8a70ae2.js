	...
	k = {
		async register(e) {
			...
			try {
				...
				Object.keys(e).forEach((a) => {
					s.append(a, e[a]), console.log(`Added to FormData: ${a} = ${e[a]}`);
				}),
					console.log("FormData string:", s.toString());
				const r = await fetch(`${l}.register_user`, {
					method: "POST",
				...
				if (
					...
					return (
						...
						{ success: !1, message: `HTTP ${r.status}: ${r.statusText}` }
					);
				let t;
				try {
					(t = await r.json()), console.log("Response data:", t);
				} catch (a) {
					console.error("Failed to parse JSON response:", a);
		...
		async login(e, o) {
			...
			try {
				...
				r.append("email", e), r.append("password", o);
				const a = await (
					await fetch(`${l}.login_user`, { method: "POST", body: r })
				).json();
				return a.message && a.message.success
					? { success: !0, message: a.message.message, data: a.message.data }
					: {
							success: !1,
							message:
								((s = a.message) == null ? void 0 : s.message) || "Login failed",
					  };
			} catch (r) {
				return (
			...
			return (t, a) => (
				...
				i(
					"input",
					{
						...
						class: m(r.value),
						onInput:
							a[0] || (a[0] = (n) => t.$emit("update:modelValue", n.target.value)),
						onBlur: a[1] || (a[1] = (n) => t.$emit("blur", n)),
						onFocus: a[2] || (a[2] = (n) => t.$emit("focus", n)),
					},
					null,
					42,
					b
				...
				return (
					...
					i(
						"button",
						{
							...
							class: m(r.value),
							onClick: a[0] || (a[0] = (u) => t.$emit("click", u)),
						},
						[
...
export { M as _, z as a, k as b, C as c };
