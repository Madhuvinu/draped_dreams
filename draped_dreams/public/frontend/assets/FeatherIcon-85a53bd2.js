import {
	...
	l as f,
	g as y,
} from "./index-68821909.js";
const l = "http://dreams.localhost:8002/api/method/draped_dreams.api.auth",
	S = {
		async register(e) {
			var o;
			try {
				...
				const a = await fetch(`${l}.register_user`, { method: "POST", body: s });
				if (
		...
		async login(e, o) {
			...
			try {
				...
				const r = await (
					await fetch(`${l}.login_user`, { method: "POST", body: a })
				).json();
		...
		async getProducts(e = {}) {
			...
			try {
				...
				const t = await (await fetch(`${l}.get_products?${s}`)).json();
				return t.message && t.message.success
		...
		async getProductDetails(e) {
			...
			try {
				...
				const t = await (
					await fetch(`${l}.get_product_details`, { method: "POST", body: s })
				).json();
				return t.message && t.message.success
					? { success: !0, data: t.message.data }
					: {
		...
		async getCategories() {
			var e;
			try {
				const s = await (await fetch(`${l}.get_categories`)).json();
...
export { M as _, z as a, S as b, C as c };
