	...
	k = {
		async register(e) {
			...
			try {
				...
				Object.keys(e).forEach((t) => {
					s.append(t, e[t]), console.log(`Added to FormData: ${t} = ${e[t]}`);
				}),
				...
				let a;
				try {
					(a = await r.json()), console.log("Response data:", a);
				} catch (t) {
					console.error("Failed to parse JSON response:", t);
				...
				return a.message && a.message.success
					...
					: {
							success: !1,
							message:
								((o = a.message) == null ? void 0 : o.message) ||
								"Registration failed",
					  };
			} catch (s) {
				return (
		...
		async getProducts(e = {}) {
			...
			try {
				...
				const a = await (await fetch(`${l}.get_products?${s}`)).json();
				return a.message && a.message.success
		...
		async getProductDetails(e) {
			...
			try {
				...
				const a = await (
					await fetch(`${l}.get_product_details`, {
						method: "POST",
						headers: { "Content-Type": "application/x-www-form-urlencoded" },
						body: s,
					})
			...
			return (a, t) => (
				...
				i(
					"input",
					{
						...
						class: m(r.value),
						onInput:
							t[0] || (t[0] = (n) => a.$emit("update:modelValue", n.target.value)),
						onBlur: t[1] || (t[1] = (n) => a.$emit("blur", n)),
						onFocus: t[2] || (t[2] = (n) => a.$emit("focus", n)),
					},
					null,
					42,
					b
			...
			const s = e,
				r = d(() => {
					const a =
						...
						u = t[s.variant] || t.primary,
						p = n[s.size] || n.md;
					return s.class ? `${a} ${u} ${p} ${s.class}` : `${a} ${u} ${p}`;
				});
			return (a, t) => {
				...
				return (
					...
					i(
						"button",
						{
							...
							class: m(r.value),
							onClick: t[0] || (t[0] = (u) => a.$emit("click", u)),
						},
						[
